...
export default function Page() {
	...
	const [filterEnglishMode, setFilterEnglishMode] = useState<Mode>(
		Mode.Containes,
	);
	const [filterLanguage, setFilterLanguage] = useState<string>("");
	const [filterLanguageMode, setFilterLanguageMode] = useState<Mode>(
		Mode.Containes,
	);
	...
	const [filterSourceNameMode, setSourceNameMode] = useState<Mode>(
		Mode.Containes,
	);
	const [filterSourceFileName, setFilterSourceFileName] =
		useState<string>("");
	const [filterSourceReference, setFilterSourceReference] =
		useState<string>("");
	const [filterSourcePublicationType, setFilterSourcePublicationType] =
		useState<string>("");
	const [filterSourceDocumentType, setFilterSourceDocumentType] =
		useState<string>("");
	const [filterSourceLocation, setFilterSourceLocation] =
		useState<string>("");
	const [filterSourceLanguage, setFilterSourceLanguage] =
		useState<string>("");
	const [filterSourceLanguageName, setFilterSourceLanguageName] =
		useState<string>("");
	...
	let displayFilterEnglish = displayFilterExtended(
		activeColumns,
		"English",
		setFilterEnglish,
		setFilterEnglishMode,
	);
	let displayFilterLanguage = displayFilterExtended(
		activeColumns,
		"Language",
		setFilterLanguage,
		setFilterLanguageMode,
	);
	let displayFilterSonetic = displayFilterInput(
		activeColumns,
		"Sonetic",
		setFilterSonetic,
	);
	let displayFilterNotes = displayFilterInput(
		activeColumns,
		"Notes",
		setFilterNotes,
	);
	let displayFilterSourceId = displayFilterInput(
		activeColumns,
		"Source ID",
		setFilterSourceId,
	);
	let displayFilterSourceName = displayFilterExtended(
		activeColumns,
		"Source Name",
		setFilterSourceName,
		setSourceNameMode,
	);
	let displayFilterSourceFileName = displayFilterInput(
		activeColumns,
		"Source File Name",
		setFilterSourceFileName,
	);
	let displayFilterSourceReference = displayFilterInput(
		activeColumns,
		"Source Reference",
		setFilterSourceReference,
	);
	let displayFilterSourcePublicationType = displayFilterInput(
		activeColumns,
		"Source Publication Type",
		setFilterSourcePublicationType,
	);
	let displayFilterSourceDocumentType = displayFilterInput(
		activeColumns,
		"Source Document Type",
		setFilterSourceDocumentType,
	);
	let displayFilterSourceLocation = displayFilterInput(
		activeColumns,
		"Source Location",
		setFilterSourceLocation,
	);
	let displayFilterSourceLanguage = displayFilterInput(
		activeColumns,
		"Source Language",
		setFilterSourceLanguage,
	);
	let displayFilterSourceLanguageName = displayFilterInput(
		activeColumns,
		"Source Language Name",
		setFilterSourceLanguageName,
	);
	let displayFilterSourceNotes = displayFilterInput(
		activeColumns,
		"Source Notes",
		setFilterSourceNotes,
	);
	...
	async function doSearch() {
		const rawResults = await searchWords({
			...
			sourceName: activeColumns.includes("Source Name")
				? filterSourceName
				: "",
			sourceFileName: activeColumns.includes("Source File Name")
				? filterSourceFileName
				: "",
			sourceReference: activeColumns.includes("Source Reference")
				? filterSourceReference
				: "",
			sourcePublicationType: activeColumns.includes(
				"Source Publication Type",
			)
				? filterSourcePublicationType
				: "",
			sourceDocumentType: activeColumns.includes("Source Document Type")
				? filterSourceDocumentType
				: "",
			sourceLocation: activeColumns.includes("Source Location")
				? filterSourceLocation
				: "",
			sourceLanguage: activeColumns.includes("Source Language")
				? filterSourceLanguage
				: "",
			sourceLanguageName: activeColumns.includes("Source Language Name")
				? filterSourceLanguageName
				: "",
			sourceNotes: activeColumns.includes("Source Notes")
				? filterSourceNotes
				: "",
		});
		...
		const newResults: string[][] = rawResults.map((result) => {
			...
			let english = formatColumn(
				activeColumns,
				"English",
				result.english,
			);
			if (english !== null) row.push(english);
			let language = formatColumn(
				activeColumns,
				"Language",
				result.language,
			);
			if (language !== null) row.push(language);
			let sonetic = formatColumn(
				activeColumns,
				"Sonetic",
				result.sonetic,
			);
	...
	return (
		<>
			...
			<SimpleTable headings={activeColumns} data={results} />
		</>
	);
}
...
function displayFilterExtended(
	activeColumns: string[],
	column: string,
	setFilter: (value: string) => void,
